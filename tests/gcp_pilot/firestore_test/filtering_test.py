...
class TestFirestoreFiltering:
    ...
    async def test_filter_greater_than_or_equal(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(integer_field__gte=30)]
        assert len(results) == 3
        for item in results:
            assert item.integer_field >= 30

    async def test_filter_less_than(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(integer_field__lt=30)]
        assert len(results) == 2
        for item in results:
            assert item.integer_field < 30

    async def test_filter_less_than_or_equal(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(integer_field__lte=30)]
        assert len(results) == 4
        for item in results:
            assert item.integer_field <= 30

    async def test_filter_in(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(integer_field__in=[10, 30, 50])]
        assert len(results) == 4
        for item in results:
            assert item.integer_field in [10, 30, 50]

    async def test_filter_not_in(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(integer_field__not_in=[10, 30, 50])]
        assert len(results) == 1
        for item in results:
            assert item.integer_field not in [10, 30, 50]

    async def test_filter_array_contains(self, populated_db):
        results = [item async for item in AllDataTypes.documents.filter(list_of_strings__contains="a")]
        assert len(results) == 3
        for item in results:
    ...
    async def test_filter_not_in_list(self):
        ...
        results = [item async for item in AllDataTypes.documents.filter(integer_field__not_in=[obj1.integer_field])]
        assert len(results) == 3
    ...
    async def test_filter_chaining(self, populated_db):
        results = [
            item async for item in AllDataTypes.documents.filter(integer_field__gte=30, list_of_strings__contains="a")
        ]
        assert len(results) == 2
        for item in results:
            assert item.integer_field >= 30
