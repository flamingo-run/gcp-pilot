...
class TestPagination:
    ...
    async def test_limit(self):
        products = [item async for item in Product.objects.filter(order_by=["price"], limit=5)]
    ...
    async def test_pagination_with_cursor(self):
        # First page
        page1 = [item async for item in Product.objects.filter(order_by=["price"], limit=10)]
        assert len(page1) == 10
        ...
        # Second page
        cursor = page1[-1]
        page2 = [item async for item in Product.objects.filter(order_by=["price"], limit=10, start_after=cursor)]
        assert len(page2) == 10
        ...
        cursor = page2[-1]
        page3 = [item async for item in Product.objects.filter(order_by=["price"], limit=10, start_after=cursor)]
        assert len(page3) == 0
    ...
    async def test_start_after_without_order_by_raises_error(self):
        page1 = [item async for item in Product.objects.filter(order_by=["price"], limit=1)]
        cursor = page1[0]
        with pytest.raises(ValueError, match="`order_by` is required when using `start_after` or `start_at`."):
            [item async for item in Product.objects.filter(start_after=cursor)]

    async def test_negative_limit_raises_error(self):
        with pytest.raises(exceptions.InvalidArgument):
            [item async for item in Product.objects.filter(order_by=["price"], limit=-1)]
    ...
    async def test_start_after_with_dict_cursor_missing_order_by_key_raises_error(self):
        ...
        with pytest.raises(ValueError):
            [item async for item in Product.objects.filter(order_by=["price"], start_after=cursor)]

    async def test_start_at_with_dict_cursor_missing_order_by_key_raises_error(self):
        cursor = {"name": "Product 5"}  # Missing "price" which is used for ordering
        with pytest.raises(ValueError):
            [item async for item in Product.objects.filter(order_by=["price"], start_at=cursor)]
    ...
    async def test_start_after_with_dict_cursor(self):
        cursor = {"price": 90}
        page = [item async for item in Product.objects.filter(order_by=["price"], limit=10, start_after=cursor)]
        assert len(page) == 10
    ...
    async def test_start_at_with_dict_cursor(self):
        cursor = {"price": 100}
        page = [item async for item in Product.objects.filter(order_by=["price"], limit=10, start_at=cursor)]
        assert len(page) == 10
        assert page[0].name == "Product 10"
        assert page[9].name == "Product 19"
    ...
    async def test_start_at_with_document_cursor(self):
        ...
        page2 = [item async for item in Product.objects.filter(order_by=["price"], limit=10, start_at=cursor)]
        assert len(page2) == 10
        assert page2[0].name == "Product 9"
